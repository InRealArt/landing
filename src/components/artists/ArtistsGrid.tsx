      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {skeletons.map((_, i) => (
          <div key={i} className="rounded-xl overflow-hidden bg-cardBackground border border-white/10">
            <div className="h-52 md:h-64 w-full bg-white/10 animate-pulse" />
  ...
  return (
    <section className="max-w-90 xl:max-w-screen-xl m-auto mt-10">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="text-white/60 text-sm">{filtered.length} {t('artists.artistsFound')}</div>
      </div>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-3 mt-8">
          <button
            ...
            className="px-3 py-2 rounded-md bg-white/10 text-white disabled:opacity-40"
          >{t('artists.previous')}</button>
          <div className="text-white/80">{t('artists.page')} {page} / {totalPages}</div>
          <button
