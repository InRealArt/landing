...
interface TeamSliderProps {
  members: Array<{
    ...
    role: string
    socials: Array<{ link: string; icon: string }>
  }>
}
...
export default function TeamSlider({ members }: TeamSliderProps) {
  ...
  const createLoopSlides = () => {
    ...
    const loopSlides = []
    const totalSlides = members.length * 2 // Dupliquer une fois
    ...
    <div className="relative bg-gradient max-w-screen-2xl m-auto mt-20">
      ...
      <section className="mt-12 relative">
        ...
        <Swiper
          ...
          spaceBetween={20}
          slidesPerView={members.length < 3 ? 1 : 2}
          speed={800}
          scrollbar={{ draggable: true }}
          initialSlide={members.length >= 3 ? members.length : 0}
          centeredSlidesBounds
          centeredSlides
          loop={members.length >= 3}
          loopAdditionalSlides={members.length >= 3 ? 2 : 0}
          ...
          breakpoints={{
            ...
            1023: {
              slidesPerView: members.length < 4 ? Math.min(members.length, 2) : 3,
            },
            1279: {
              slidesPerView: members.length < 5 ? Math.min(members.length, 3) : 4,
            },
          }}
        >
      ...
      <style jsx global>{`
        .swiper-wrapper {
          ${members.length < 3 ? 'justify-content: center !important;' : ''}
        }
        
        .swiper-slide {
          ${members.length < 3 ? 'width: auto !important;' : ''}
        }
        
        .swiper {
          ${members.length < 3 ? 'width: fit-content !important; margin: 0 auto !important;' : ''}
        }
