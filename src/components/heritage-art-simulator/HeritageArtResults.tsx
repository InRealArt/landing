  ...
  return (
    <div className="space-y-6">
      ...
      <div className="border-l-2 border-[var(--gold-accent)] pl-5 py-2">
        <p className="text-xs uppercase tracking-[0.25em] text-[var(--gray-text)] mb-3">
          {t('heritageArtSimulator.results.recommendation')}
        </p>
        <div
          className="text-sm text-[var(--ink-black)] leading-loose"
      ...
      <div className="border border-[var(--border-light)] p-5">
        <h3 className="text-xs uppercase tracking-[0.25em] text-[var(--gray-text)] mb-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-0 border border-[var(--border-light)]">
        <div className="p-5 border-b md:border-b-0 md:border-r border-[var(--border-light)]">
          <h3 className="text-xs uppercase tracking-[0.25em] text-[var(--gray-text)] mb-4">
          ...
          <div className="divide-y divide-[var(--border-light)]">
            {(['immobilier', 'liquidites', 'financier', 'crypto', 'tangibles'] as const).map(field => (
              <div key={field} className="flex justify-between items-center py-2.5">
                <span className="text-sm text-[var(--gray-text)]">{t(`heritageArtSimulator.form.${field}`)}</span>
                <span className="text-sm text-[var(--ink-black)] font-medium">{results.patrimoineActuel[field]}%</span>
              </div>
            ))}
          </div>
        </div>
        <div className="p-5">
          ...
          <div className="divide-y divide-[var(--border-light)]">
            {(['immobilier', 'liquidites', 'financier', 'crypto', 'tangibles'] as const).map(field => (
              <div key={field} className="flex justify-between items-center py-2.5">
                <span className="text-sm text-[var(--gray-text)]">{t(`heritageArtSimulator.form.${field}`)}</span>
                <span className="text-sm text-[var(--ink-black)] font-medium">{results.repartitionAjustee[field]}%</span>
              </div>
            ))}
            <div className="flex justify-between items-center py-2.5 bg-[var(--ink-black)] -mx-5 px-5 mt-1">
              <span className="text-sm text-white/60 uppercase tracking-[0.15em]">{t('heritageArtSimulator.results.artRecommended')}</span>
      ...
      <div className="border border-[var(--border-light)]">
        <div className="px-5 py-3 border-b border-[var(--border-light)] bg-[var(--soft-gray)]">
          <h3 className="text-xs uppercase tracking-[0.25em] text-[var(--gray-text)]">
            {t('heritageArtSimulator.results.profileSummary')}
          </h3>
        </div>
        <div className="grid grid-cols-2 divide-x divide-[var(--border-light)]">
          <div className="px-5 py-4">
            <p className="text-xs text-[var(--gray-text)] mb-1">{t('heritageArtSimulator.form.profil')}</p>
            <p className="text-sm text-[var(--ink-black)] font-medium">{results.profil}</p>
          </div>
          <div className="px-5 py-4">
            <p className="text-xs text-[var(--gray-text)] mb-1">{t('heritageArtSimulator.form.objectif')}</p>
            <p className="text-sm text-[var(--ink-black)] font-medium">{results.objectif}</p>
          </div>
        </div>
      </div>
