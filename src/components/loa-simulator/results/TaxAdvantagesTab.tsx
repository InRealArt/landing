...
export default function TaxAdvantagesTab({ leaseResults }: TaxAdvantagesTabProps) {
  ...
  return (
    <div className="bg-backgroundColor/30 backdrop-blur-sm rounded-2xl p-6 border border-white/20 shadow-2xl">
      <h2 className="text-2xl font-bold text-textColor mb-6 font-bricolage">{t('loaSimulator.results.tax.title')}</h2>
      ...
      <div className="space-y-6">
        <div className="text-center bg-gradient-to-r from-green-900/30 to-emerald-900/30 rounded-xl p-6 border border-green-500/30">
          <div className="w-16 h-16 mx-auto mb-4 bg-gradient-to-r from-green-500 to-green-600 rounded-full flex items-center justify-center">
            <PiggyBank className="w-8 h-8 text-textColor" />
          </div>
          <h3 className="text-3xl font-bold text-textColor mb-2 font-bricolage">
            {formatCurrency(leaseResults.taxSavings)}
          </h3>
          <p className="text-textColor/70 font-unbounded">{t('loaSimulator.results.tax.totalTaxSavings')}</p>
        </div>

        <div className="bg-backgroundColor/10 rounded-xl p-4">
          <h3 className="text-lg font-semibold text-textColor mb-4 font-bricolage">{t('loaSimulator.results.tax.taxAdvantagesDetail')}</h3>
          
          <div className="space-y-3">
            <div className="flex justify-between py-2 border-b border-white/20">
              <span className="text-textColor/70 font-unbounded">{t('loaSimulator.results.tax.monthlyDeduction')}</span>
              <span className="text-green-300 font-semibold font-unbounded">{formatCurrency(leaseResults.monthlyTaxDeduction)}</span>
            </div>
            
            <div className="flex justify-between py-2 border-b border-white/20">
              <span className="text-textColor/70 font-unbounded">{t('loaSimulator.results.tax.totalDeductibleRents')}</span>
            ...
            <div className="flex justify-between py-2 bg-green-900/30 px-4 rounded-lg border border-green-500/30">
              <span className="text-textColor font-medium font-unbounded">{t('loaSimulator.results.tax.totalTaxSavingsLabel')}</span>
